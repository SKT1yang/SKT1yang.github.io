...
export default function Home() {
  const [fireworks, setFireworks] = useState(initialFireworks)
  const [filteredFireworks, setFilteredFireworks] = useState(initialFireworks)
  ...
  return (
    <div className="min-h-screen bg-festive-bg">
      ...
      <main className="container mx-auto px-4 py-8">
        <CategoryFilter selectedCategory={selectedCategory} onCategoryClick={handleCategoryClick} />
        {filteredFireworks.length === 0 ? (
          <div className="text-center mt-8">
            <Sparkles className="inline-block text-red-600 mb-4" size={48} />
            <p className="text-2xl font-bold text-red-600">没有找到匹配的烟花</p>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-8">
              {currentFireworks.map((firework) => (
                <FireworkCard key={firework.id} {...firework} />
              ))}
            </div>
            {totalPages > 1 && (
              <Pagination
